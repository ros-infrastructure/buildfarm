...
def parse_options():
    ...
    parser.add_argument('--repo',
            help='The git local build package repo to work with.' + 
            ' Will be created if it doesn\'t exist')
    parser.add_argument('--upstream',
            help='Must be a tar ball of the form <project>_<version>.orig.tar.gz')
...
def repo_call(repo, call, pipe=None):
    print('+ ' + ' '.join(call))
    process = Popen(call, stdout=pipe, cwd=repo)
    output, unused_err = process.communicate()
    retcode = process.poll()
    if retcode:
        raise CalledProcessError(retcode, call,
         output=output)
    if pipe:
...
def novel_version(repo, version):
    if not os.path.isdir(repo):
        return True
    tags = repo_call(repo, ['git', 'tag'], pipe=subprocess.PIPE)
...
def import_orig(repo, upstream, version):
    ...
    if len(repo_call(repo, ['git', 'diff'], pipe=subprocess.PIPE)):
        repo_call(repo, ['git', 'commit', '-a', '-m', '"commit all..."'])
    repo_call(repo, ['git', 'import-orig', upstream])
...
''' % locals()
    ...
    repo_call(repo, ['git', 'buildpackage',
        '-S', '--git-export-dir=%s' % deb_path,
        '--git-ignore-new', '--git-retag', '--git-tag', tag]
        + signed.split(' '))
